...
`WIFI_SETUP_INIT`（初回起動時）・`WIFI_SETUP_SETTINGS`（SETTINGS画面からのWi-Fi再設定）・`WIFI_SETUP_RECONNECT`（機体情報再取得時のWi-Fi接続失敗からの復帰）のいずれも、同一のタイムアウト仕様を適用する。従来`WIFI_SETUP_RECONNECT`のみ「リトライ機構の対象外」としていたが、リトライ機構自体を廃止したため、3フローとも同じ挙動（1回の試行→失敗時は`BACK`のみ）に統一される。

**画面表示：**

* ボタンは常に`BACK`のみを表示する（`RETRY`ボタン・リトライ回数表示は設けない）。
* `BACK`押下時は入力フォームへ戻り、SSID・パスワード等を入力し直す。
...
| **右端（BtnC）** | **進む・決定** | `SET`、`SELECT`、`CONFIRM`、`NEXT`、`RETRY` |
...
同じ画面でも遷移元によって決定後の動作が変わるものがある（確認ダイアログ：5.7.2参照、SETTINGS：5.7参照）。この場合、遷移元を記録する変数を個別に用意する（いずれも、遷移元ごとに個別のenum値を用意する方式を採用しており、汎用的な「直前の画面」変数を避けるという考え方は共通している）。

汎用的に「直前の画面」を全画面で保持する方式（`previousMode`等）も考えられるが、以下の理由から現時点では採用しない。

* 分岐が必要な画面はごく一部（現状2つ）にとどまり、変数名から意図が読み取れる個別方式の方が可読性が高い。

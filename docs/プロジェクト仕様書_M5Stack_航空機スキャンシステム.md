...
本節の判定のうち、**Wi-Fi接続の判定のみが実装済み**である（`initStateMachine()`、3.2.2参照）。APIキー・取得地点の登録判定、およびキャッシュ有無によるAPIリクエストの省略判定は8.2-Dで実装予定であり、現時点では`main.cpp`の一時テストコードが、Wi-Fi接続成功時に無条件でAPIキー・取得地点を仮登録して機体情報を取得している。
...
現在の`saveCache()`は機体情報と残りリクエスト数のみを保存しており、**取得日時（`lastUpdateTime`、5.5参照）を保存していない**。`lastUpdateTime`はRAM上の変数でNTP同期成功時にのみ設定されるため、再起動後にキャッシュを表示すると`Updated : --/-- --:--`と空欄になる。

現時点ではWi-Fi接続失敗時にのみ顕在化するが、8.2-Dで「キャッシュがあればAPIリクエストを省略する」本来の起動フロー（1.1参照）を実装すると、通常起動でも常に発生する。その際、`cache.json`に`lastUpdateTime`を追加し、`saveCache()`で保存・`loadCache()`で復元する対応が必要となる（M5Stack BasicにはRTCがないため、時刻そのものを本体に保持する方法は使えない）。
...
* **初期値（未取得時）**：`"--/-- --:--"` とする。
* **未対応の課題**：キャッシュ表示時は常に初期値（`--/-- --:--`）となる。`lastUpdateTime`がキャッシュに保存されていないためであり、対応方針は2.4節を参照。
...
│   ├── ui_handler_wifi.cpp       // Wi-Fi設定関連画面の描画（5.9参照、手順25で追加）
│   ├── storage_handler.cpp
│   ├── state_machine.cpp
│   ├── system_status.cpp
│   └── input_handler.cpp
...
**補足：** `main.cpp`から`ui_handler`・`storage_handler`・`flight_data`への依存は、手順25で恒久的なものになった（接続中画面の描画・キャッシュの復元）。従来は一時テストコード由来の破線だった。
...
28. 基準地点設定ページ（`location.html`）の作成と登録フローの実装（Leaflet地図・数値入力の2方式。4.1参照） ✅
    ...
    * 以下は本手順の当初スコープに含めていたが、別途対応することとし、8.2-Dの本手順からは切り離した（未実装のまま保留）
      * 起動時分岐へのAPIキー・基準地点の登録状況による判定の追加、および`main.cpp`の一時テストコードの撤去（1.1参照）
      * キャッシュ有無によるAPIリクエスト省略判定、および`lastUpdateTime`のキャッシュ保存（2.4参照）
...
* キャッシュ表示時の取得日時（`lastUpdateTime`）の保存対応（2.4参照、8.2-Dで対応予定）

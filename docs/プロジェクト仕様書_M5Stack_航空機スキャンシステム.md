...
> **【注記：CoreS3対応の固定について（2026-08-27追記）】**
> 開発機材の都合によりCoreS3実機を継続して利用できなくなったため、CoreS3対応は`v1.0.0`時点（`cores3-v1.0.0`ブランチ）で固定した。以降`main`ブランチはM5Stack Basic専用として開発しており、CoreS3での動作は保証しない。本仕様書中のCoreS3関連の記述（7.0.2の初期化順序の注意、7.0.3の仮想ボタン対応、7.2のplatformio.iniにおけるCoreS3向け環境、3.2.2の再起動方式の採用理由等）は、いずれも`v1.0.0`時点の内容として参照すること。
...
本節の判定はすべて実装済みである。Wi-Fi接続判定・APIキー/取得地点の登録判定（`initStateMachine()`での起動時分岐、`QrCaller`による`handleQrView()`の連鎖遷移。3.2.2・8.2-D参照）に加え、**キャッシュ有無によるAPIリクエストの省略判定も実装済み**である。`initStateMachine()`で`main.cpp`から渡される`loadCache()`の戻り値（キャッシュの有無）を判定し、キャッシュがあればAPIリクエストを行わず、`disableWiFi()`の上で機体情報表示フローへ直接遷移する（8.2-D参照）。`main.cpp`の一時テストコードは撤去済みである。
...
* 収録データがICAOコードをキーとして構成されているため（データ出典・収録件数は5.3.1参照）。
...
**検索方式：**
* `strcmp`ベースの**二分探索**を用いる。件数が増えても比較回数はlog2的にしか増えないため、現行の99件はもちろん、件数が今後増えても処理時間への影響は無視できる（API通信1回の0.2〜0.5秒と比べて桁違いに軽い）。
...
|---|---|---|
| 現行 | 国際79社＋国内20社の計99社 | 実測：Basic機でFlash使用率59.2%（1,241,341/2,097,152バイト、`no_ota.csv`採用時） |
| 拡張時（必要に応じて） | IATA公式データに基づき追加収録 | 追加分の概算は1社あたり数十バイト程度（実測値から按分） |

* まず主要20〜30社に絞った状態で実装し、使用感を見て必要に応じて収録範囲を拡大する。
* **収録件数の見直し経緯**：当初は主要50〜100社を想定していたが、手順22完了時点でフラッシュ使用率が92.1%に達していたことが判明し（暫定実装10社の時点で既にこの水準）、想定より厳しいフラッシュ余地であることを踏まえて20〜30社へ絞り込む方針に変更した。あわせて、OTA不使用を前提としたパーティション構成の見直し（Basic：`no_ota.csv`、CoreS3：`default_16MB.csv`、7.2参照）によりアプリ領域自体を拡張する対応も行っている。
* **データ取得元の変更**：当初の収録データ（`airline_icao.csv`）は個人開発プロジェクトでの利用が利用規約上問題となる可能性があったため、取得元を国土交通省 航空局「航空会社コード一覧」（国内）およびIATA公式「Current Airline Members」（国際）へ変更し、手作業でのコピー＆ペーストによりデータを収集する方針とした（自動スクレイピング等は行っていない）。
* **収録件数の再拡大**：新データソースへの切り替え後、国際線就航実績（主要6空港）に基づき抽出した79社と、国内20社（全24社から運航休止・コード不明等の4社を除外）をあわせた計99社で実装した。Basic機での実機ビルドにより、Flash使用率59.2%（1,241,341/2,097,152バイト）、RAM使用率16.2%と、20〜30社時点から実用上問題ない範囲に収まることを確認済み。
...
* 収録件数が数百社規模まで拡大する場合は、手作業でのC++配列変換が非現実的になるため、変換スクリプト（Python等）による自動生成を検討する。現行の99社規模では手作業での変換で対応している。
...
28. 基準地点設定ページ（`location.html`）の作成と登録フローの実装（Leaflet地図・数値入力の2方式。4.1参照） ✅
    ...
    * 起動時分岐へのAPIキー・基準地点の登録状況による判定を追加し、`main.cpp`の一時テストコードを撤去した（1.1参照） ✅
      ...
      * `initStateMachine()`で、Wi-Fi接続成功時に`loadApiKey()`・`loadConfig()`（`LOCATION_UNSET`判定）を行い、未登録の項目があれば`MODE_QR_VIEW`（`QR_CALLER_INIT`）へ、両方登録済みなら次項のキャッシュ有無判定へ遷移するよう実装した
    ...
    * キャッシュ有無によるAPIリクエスト省略判定を実装した（1.1・2.4参照） ✅
      * `main.cpp`で起動時に`loadCache()`の戻り値（キャッシュの有無）を取得し、`initStateMachine()`へ引数として渡す方式とした
      * `initStateMachine()`の「APIキー・基準地点とも登録済み」分岐に、キャッシュ有無による分岐を追加した。キャッシュありの場合はAPIリクエストを行わず、`disableWiFi()`の上で`totalFlightCount`に応じて`MODE_FLIGHT_VIEW`または`MODE_NO_FLIGHTS_VIEW`へ直接遷移する。キャッシュなしの場合は従来通り`MODE_LOADING`へ遷移する
      * 初回登録完了直後（`QR_CALLER_INIT`経由）のデータ取得、およびFLIGHT_VIEWからの手動再取得（`CONFIRM_REFRESH`経由）は、いずれもキャッシュ有無の判定対象外とした（初回はキャッシュが存在せず、手動再取得はユーザーの明示的な意図によるデータ更新のため）
      * 電源ON・再起動時（キャッシュあり／機体0件のキャッシュあり、いずれもAPIリクエストを省略して直接遷移）、FLIGHT_VIEWからの手動再取得（キャッシュ有無に関わらず必ずAPIリクエスト）、RESET ALL後の再登録フロー（キャッシュなし扱いで通常通りAPIリクエスト）、いずれのケースも実機検証済み

**E. データ整備**

29. 航空会社コード変換辞書の実装（`const AirlineEntry[]`静的配列＋二分探索、国際79社＋国内20社の計99社で実装、必要に応じてIATA公式データに基づき追加収録。5.3.1参照）

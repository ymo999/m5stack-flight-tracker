...
|---|---|---|---|
| `flight_icao` | 便名（ICAOコード） | 例: "SKY598" | **画面上部にメイン表示**。あわせてFlightAware用URL（`https://ja.flightaware.com/live/flight/{flight_icao}`）のQRコード生成にも使用（5.3参照） |
...
ESP32のフラッシュメモリには書き込み回数の上限（一般に10,000〜100,000回程度）がある。NVS・LittleFSはいずれも同一のフラッシュ上に構築されるため、どちらも制限の対象となる。
...
| ポータルが自動表示されない事例 | ESP32環境でキャプティブポータルがポップアップしない報告がある | Issue #553 |
...
* **フォントサイズは変更しない**：文字数に応じてフォントを縮小する方式は採用しない（デザインの一貫性・視認性を優先するため）。

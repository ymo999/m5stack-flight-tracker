...
/**
 * @def MAX_FLIGHT_COUNT
 * @brief 保持する機体データの上限
 */
#define MAX_FLIGHT_COUNT 10

/**
 * @brief 航空機1機分のデータを保持する構造体
 * 
 * 文字列項目...空文字列
 * 数値項目...0の場合、当該項目を欠損値として扱う
 * 
 * @note ただし @p alt, @p speed, @p heading については0が実測値となるケースもありうるため、-1を欠損値として扱う
 */
struct FlightData {
...
extern FlightData foundFlights[MAX_FLIGHT_COUNT];   ///< 航空機データの配列
extern int totalFlightCount;                        ///< 総件数
extern int currentDisplayIndex;                     ///< 現在の表示インデックス
